...
export async function listInspectionPlans(pool, profile, organizationId) {
  ...
  const result = await pool.query(`SELECT plan.*,unit.unit_code,unit.status AS asset_status,
    ...
    LEFT JOIN LATERAL (SELECT inspected_at,status FROM logistics_inspection_runs inspection
      WHERE inspection.asset_unit_id=unit.id AND inspection.status IN ('APPROVED','CLOSED')
...
export async function upsertInspectionPlan(pool, input, actorProfileId = null) {
  ...
  try {
    ...
    const unit = (await client.query(`SELECT id FROM logistics_asset_units
      WHERE id=$1 AND organization_id=$2 AND status NOT IN ('RETIRED','LOST')`,
    ...
    const result = await client.query(`INSERT INTO logistics_asset_inspection_plans
      (organization_id,asset_unit_id,interval_days,warning_days,grace_days,initial_due_at,
       ...
       block_on_overdue,active,notes,created_by,updated_by,updated_at)
      VALUES ($1,$2,$3,$4,$5,$6,$6,$7,$8,$9,$10,$11,$12,$12,NOW())
      ...
      input.active !== false, text(input.notes) || null, actorProfileId]);
