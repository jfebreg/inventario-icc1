...
export async function createInboundReceipt(pool, input, actorProfileId = null) {
  ...
  try {
    ...
    for (const [index, lineInput] of lines.entries()) {
      ...
      const purchaseUnitCost = Math.max(0, number(lineInput.unitCost, number(item.standard_cost)));
      const currency = text(lineInput.currency || item.currency || "CLP").toUpperCase();
      ...
      const line = (await client.query(`INSERT INTO logistics_inbound_receipt_lines
        ...
        VALUES ($1,$2,$3,$4,$5,'QUARANTINE',$6,$7,$8,$9) RETURNING *`,
        [receipt.id, itemId, assetUnitId, lotId, quantity, posted.movement.id,
          purchaseUnitCost, currency, text(lineInput.notes) || null])).rows[0];
