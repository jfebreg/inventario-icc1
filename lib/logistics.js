...
export async function evaluateOutboxDeliverySlo(pool, organizationId) {
  ...
  const existing = (await pool.query(`SELECT id,status FROM inventory_tasks WHERE id=$1`, [taskId])).rows[0];
  ...
  if (reasons.length) {
    ...
    await pool.query(`INSERT INTO inventory_tasks
      ...
      VALUES ($1,'OUTBOX_HEALTH','Recuperar entrega de eventos',$2,'Crítica','Pendiente',
        'Bodega Central','outbox_monitor',$3,NOW()+INTERVAL '4 hours',$4::jsonb,NOW())
      ON CONFLICT (id) DO UPDATE SET detail=EXCLUDED.detail,priority='Crítica',
        ...
        resolved_at=NULL,due_at=COALESCE(inventory_tasks.due_at,EXCLUDED.due_at),
        payload=EXCLUDED.payload,updated_at=NOW()`,
    [taskId, reasons.join(" "), policy.id, json(snapshot)]);
    if (newlyBreached) {
      await pool.query(`INSERT INTO inventory_notifications
        (id,center_name,notification_type,title,body,severity,entity_type,entity_id,payload)
        VALUES ($1,'Bodega Central','OUTBOX_HEALTH_BREACH','Entrega de eventos degradada',$2,
          'critical','outbox_monitor',$3,$4::jsonb)
        ON CONFLICT (id) DO UPDATE SET body=EXCLUDED.body,severity='critical',read_at=NULL,
          payload=EXCLUDED.payload`,
      [`notification-${taskId}`, reasons.join(" "), policy.id, json(snapshot)]);
      await pool.query(`INSERT INTO logistics_audit_events
    ...
    return { status: "BREACHED", reasons, policy, metrics: snapshot };
  }
