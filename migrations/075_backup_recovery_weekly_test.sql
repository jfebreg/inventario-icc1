-- Prueba semanal automática de recuperabilidad del último respaldo canónico.
...
ALTER TABLE logistics_scheduled_jobs
  ...
  CHECK (job_code IN (
    ...
    'BACKUP_RECOVERY_WEEKLY_TEST'
  ));

INSERT INTO logistics_scheduled_jobs
  (organization_id,job_code,enabled,timezone_name,local_hour,period_days,next_run_at)
SELECT id,'BACKUP_RECOVERY_WEEKLY_TEST',TRUE,'America/Santiago',5,7,
  ((((NOW() AT TIME ZONE 'America/Santiago')::date+1)+INTERVAL '5 hours')
